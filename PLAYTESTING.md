# Playtesting checklist, Mutil Refreshed
...
* [ ] Tested through a consumer such as Tetra Refreshed before testing inside a full pack
...
This is a library and does nothing visible on its own, so it is exercised through a consumer. Tetra
Refreshed covers the most of it.

* [ ] A consumer builds against it and loads
* [ ] Its data stores report the counts the consumer expects
* [ ] Screens built on its gui toolkit draw and close without error
* [ ] Packets it registers cross in both directions with no missing handler on either side
...
* [ ] Works with anything that hooks into the data, network and gui helpers it provides
